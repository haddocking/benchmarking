...
- **Molecule suffixes**: `_r_b` (receptor, bound), `_l_b` (glycan, bound), `_r_u` (receptor, unbound), `_l_u` (glycan, unbound), `_l_ensemble` (glycan conformational ensemble for `scenario_unbound_ens_vdw_tipap_clust`)
...
### scenario_unbound_ens_vdw_tipap_clust

An extended version of `scenario_unbound_vdw_tip_ap` that incorporates ensemble-based sampling. Multiple conformations of the unbound glycan (provided as a structural ensemble) are used during the rigid-body stage to better represent the conformational space accessible to the free glycan.
