...
ALL_PERFORMANCES_CLASSES = {
    ...
    # NOTE: protein-glycan is assessed with ilrmsd (the default metric for this
    # type); irmsd is not used. dockq follows the protein-peptide convention.
    "glycan": {
        "dockq": {
            "High": (0.895, 1),
            "Medium": (0.71, 0.895),
    ...
    #irmsd and dockq values for dna
    "dna": {
        "irmsd": {
            "High": (0, 1),
            "Medium": (1, 2),
    ...
    # NOTE: protein-ligand (small molecule) is assessed with ilrmsd, same as
    # glycan
    "small_molecule": {
        "ilrmsd": {
            "High": (0, 1),
...
COLORS_MAPPER = {
    ...
    "Low": "lightcoral",     # was "white" (invisible on white background)
    "Missing": "dimgrey",
    }
...
def stage_name(cname: str) -> str:
    ...
    Preference order:
      1. auto-detected module name (the module this caprieval evaluated),
      2. the hardcoded CAPRIEVAL_STEPS mapping,
      3. a `<index>_caprieval` fallback.
    ...
    name : str
        Name of the stage
    """
    if cname in DETECTED_STAGE_NAMES:
        return DETECTED_STAGE_NAMES[cname]
    try:
        return CAPRIEVAL_STEPS[cname]
    except KeyError:
        return f"{cname}_caprieval"


def detect_stage_modules(
        dtmapper: dict,
        read_from_archive: bool = False,
        ) -> dict[str, str]:
    """Auto-name caprieval stages by the module they evaluated.

    Every caprieval `capri_ss.tsv` lists model paths like
    `../../10_seletopclusts/cluster_1_model_1.pdb`; the parent directory
    (`10_seletopclusts`) is the module being evaluated. This reads one file
    per stage (from the first scenario/target) and strips the numeric prefix
    to get the module name (`seletopclusts`). Duplicate module names across
    stages are disambiguated with their stage index, e.g. `flexref (07)`.

    Falls back silently (empty entry) for any stage it cannot read, so the
    hardcoded CAPRIEVAL_STEPS / `<index>_caprieval` naming still applies.
    """
    mapping: dict[str, str] = {}
    if not dtmapper:
        return mapping
    scen = sorted(dtmapper)[0]
    for stage, pdbmap in dtmapper[scen].items():
        if not pdbmap:
            continue
        first_pdb = next(iter(pdbmap))
        try:
            data = load_caprieval_data(pdbmap[first_pdb], read_from_archive)
            model_path = next(iter(data))
            module_dir = os.path.basename(os.path.dirname(str(model_path)))
            module = re.sub(r'^\d+_', '', module_dir)
            if module:
                mapping[stage] = module
        except Exception:
            continue  # leave undetected; stage_name() falls back
    # disambiguate repeated module names by appending the stage index
    counts: dict[str, int] = {}
    for m in mapping.values():
        counts[m] = counts.get(m, 0) + 1
    for stage in list(mapping):
        if counts[mapping[stage]] > 1:
            mapping[stage] = f"{mapping[stage]} ({stage})"
    return mapping
...
def make_scenar_melquiplots(
    ...
    fig.suptitle(title, fontsize=16)
    # Add Legend
    # Metric-aware label: distance metrics (irmsd/ilrmsd) are in Angstrom and
    # lower-is-better (`lo <= M < hi`); dockq is unitless and higher-is-better
    # (`lo < M <= hi`), so both the unit and the inequality direction flip.
    higher_is_better = get_reverse_bool(perf_dtype)
    unit = "" if higher_is_better else r"$\AA$"

    def _perf_label(perfclass: str) -> str:
        lo, hi = dtype_perf_classes[perfclass]
        metric = perf_dtype.upper()
        if higher_is_better:
            return rf"{perfclass} | {lo} < {metric} <= {hi}{unit}"
        return rf"{perfclass} | {lo} <= {metric} < {hi}{unit}"

    legend_data = [
        (
            plt.Rectangle((0, 0), 1, 1, fc=COLORS_MAPPER[perfclass]),
            _perf_label(perfclass),
            )
...
def _find_scenario_data_dir(
    ...
    for entry in Path(candidate_path).glob("*/"):
        # A genuine target holds either a `run1/` run directory (default,
        # non-archive mode) or a `run1_analysis.tgz` archive (`-a` mode).
        # Accept both so scenario discovery works regardless of which mode
        # the benchmark was produced/analysed in.
        if entry.is_dir() and (
                (entry / "run1").is_dir()
                or (entry / "run1_analysis.tgz").is_file()
                ):
...
def main(
    ...
    DETECTED_STAGE_NAMES = detect_stage_modules(dtmapper, read_from_archive)
    if DETECTED_STAGE_NAMES:
        vprint(
            "- Auto-named caprieval stages: "
            + ", ".join(f"{k}->{v}" for k, v in sorted(DETECTED_STAGE_NAMES.items()))
            )
    ...
    # Plot title: when the user passes -l/--label, use it verbatim as the
    # title. Otherwise fall back to a plain generic title. (Note: `basename`
    # is still used for output filenames, so it stays as-is above.)
    plot_title = label if label else "Benchmark output"
...
def parse_cmd_line() -> argparse.Namespace:
    ...
    # Type-aware default metric: glycan and small molecule are assessed with
    # ilrmsd by default; every other type defaults to irmsd. An explicit -m
    # always wins.
    if args.metric is None:
        args.metric = "ilrmsd" if args.type in ("glycan", "small_molecule") else "irmsd"
    # Set global variables
    DPI = args.dpi
    PERFORMANCES_CLASSES = ALL_PERFORMANCES_CLASSES[args.type]
    # `ilrmsd` boundaries are only defined for protein-glycan. Reject any other
    # combination early with a clear message rather than crashing on a KeyError.
    if args.metric not in PERFORMANCES_CLASSES:
        sys.exit(
            f'INPUT ERROR: metric "{args.metric}" is not defined for type '
            f'"{args.type}". Available metrics for "{args.type}": '
            f'{", ".join(PERFORMANCES_CLASSES.keys())}. '
            f'(Note: `ilrmsd` is only for `-t glycan` and `-t small_molecule`.)'
            )
    ...
    # Echo the resolved configuration so every run states exactly which
    # metric it is classifying by (and which are available for this type).
    if not args.quiet:
        default_note = " (default for this type)" if args.metric == (
            "ilrmsd" if args.type in ("glycan", "small_molecule") else "irmsd"
            ) else " (explicitly set via -m)"
        print(
            f"[config] type = {args.type}  |  metric = {args.metric}"
            f"{default_note}  |  metrics available for {args.type}: "
            f"{', '.join(PERFORMANCES_CLASSES.keys())}"
            )
    return args
...
def _get_cmd_line_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "--metric",
        help=(
            "Performance metric to track. If not given, defaults to `ilrmsd` "
            "for `-t glycan` and `-t small_molecule`, and `irmsd` otherwise. "
            "`ilrmsd` is only defined for glycan and small molecule."
            ),
        required=False,
        default=None,
        choices=("irmsd", "dockq", "ilrmsd", ),
        type=str,
        )
    parser.add_argument(
        '-s',
    ...
    parser.add_argument(
        ...
        required=False,
        default="protein",
        choices=("protein", "peptide", "glycan", "dna", "small_molecule", ),
        type=str,
        )
    parser.add_argument(
        '-d',
