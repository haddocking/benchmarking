...
NOTE: bar plots are generated by default. Violin and melqui plots are OFF by
default -- enable them explicitly with --violinplots and/or --melquiplots.
Default figure DPI is 200.
"""
import argparse
import glob
import json
...
# Performance classes shown in the plot legends. `Low` and `Missing` are
# intentionally omitted (both are drawn white / effectively invisible), so
# they don't clutter the legend. Bars are still stacked using PERF_ORDER.
LEGEND_PERF_ORDER = tuple(
    p for p in PERF_ORDER if p not in ("Low", "Missing")
    )
...
def stage_name(cname: str) -> str:
    ...
    Preference order:
      1. an explicit CAPRIEVAL_STEPS entry (manual override wins),
      2. the auto-detected module name (the module this caprieval evaluated),
    ...
    name : str
        Name of the stage
    """
    # A manually-specified name always wins, so CAPRIEVAL_STEPS can be used
    # to override the auto-detected module name for any stage.
    if cname in CAPRIEVAL_STEPS:
...
def make_scenar_melquiplots(
    ...
    legend_data = [
        ...
        for perfclass in LEGEND_PERF_ORDER
        ]
...
def main(
    ...
    no_capriplots : bool, optional
        ...
        by default False
    violinplots : bool, optional
        Generate the violin plots. OFF by default.
    melquiplots : bool, optional
        Generate the melquiplots. OFF by default.
    read_from_archive : bool, optional
        When set to True, performs the search of capri_ss.tsv files from the
        analysis archive instead of the run directory, by default False
    ...
    if DETECTED_STAGE_NAMES:
        # Show the EFFECTIVE label per stage (a CAPRIEVAL_STEPS entry
        # overrides the auto-detected name), so the log matches the plots.
        vprint(
            "- Caprieval stage names: "
            + ", ".join(
                f"{k}->{stage_name(k)}"
                for k in sorted(DETECTED_STAGE_NAMES)
                )
            )
    ...
    plot_title = label if label else f"Benchmark output - {metric}"
    # Draw general graph
...
def _get_cmd_line_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "(one or more scenarios). Generates a combined bar plot by "
            "default; violin plots and per-scenario melquiplots (zipped) "
            "are optional (--violinplots / --melquiplots), plus a JSON "
    ...
    parser.add_argument(
        '--violinplots',
        help="Also generate violin plots (off by default).",
        action="store_true",
        default=False,
        )
    parser.add_argument(
        '--melquiplots',
        help="Also generate melqui plots (off by default).",
        action="store_true",
        default=False,
        )
    parser.add_argument(
